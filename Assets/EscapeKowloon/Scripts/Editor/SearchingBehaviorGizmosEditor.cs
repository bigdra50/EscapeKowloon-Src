...
public static class SearchingBehaviorGizmosEditor
{
    ...
    private static void DrawPointGizmos(SearchTargetVisibility i_object, GizmoType i_gizmoType)
    {
        ...
        {
            Mesh fanMesh = CreateFunMesh(i_object.SearchAngle, TRIANGLE_COUNT);

            Gizmos.DrawMesh(fanMesh, pos, rot, scale);
        }
    }

    private static Mesh CreateFunMesh(float i_angle, int i_triangleCount)
    {
        var mesh = new Mesh();
